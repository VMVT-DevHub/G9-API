...
DO LANGUAGE 'plpgsql' $$ BEGIN
	...
	CREATE TABLE jar.data (ja_kodas int NOT NULL, ja_pavadinimas varchar(255) NOT NULL, adresas varchar(255), aob_kodas int, form_kodas int, form_pavadinimas varchar(255),status_kodas int,reg_data date, CONSTRAINT data_pkey PRIMARY KEY (ja_kodas));
...
DO LANGUAGE 'plpgsql' $$ BEGIN
	...
	CREATE OR REPLACE VIEW g9.v_gvts_ja AS SELECT g.vkl_id AS gvts, j.ja_kodas AS "ID", j.ja_pavadinimas AS "Title", j.adresas AS "Addr" FROM g9.gvts g LEFT JOIN jar.data j ON g.vkl_ja = j.ja_kodas WHERE j.ja_kodas IS NOT NULL;
