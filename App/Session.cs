...
public static class Session {
	...
	private static void AddCookie(UserSession sess, HttpContext ctx){
		...
		ctx.Response.Cookies.Append("SSID", sess.SSID, new CookieOptions(){ SameSite=SameSiteMode.Strict, HttpOnly=true, Secure=true});
	}
...
public class UserSession {
	...
	public DateTime Expire { get; set; } = DateTime.UtcNow.AddSeconds(Config.GetLong("Session", "Extend", 60));
	/// <summary>Sesijos pratęsimo laikas</summary>
	public DateTime Extend { get; set; } = DateTime.UtcNow.AddSeconds(Config.GetLong("Session", "Expire", 300));
