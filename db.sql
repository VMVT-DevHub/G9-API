...
--Public Tables:	
	...
	CREATE TABLE IF NOT EXISTS public.deklaravimas (dkl_id integer NOT NULL GENERATED ALWAYS AS IDENTITY, dkl_gvts bigint NOT NULL, dkl_metai integer NOT NULL, dkl_stebesena integer NOT NULL, dkl_status integer NOT NULL, dkl_kiekis double precision, dkl_vartot integer, dkl_deklar_date timestamp(3), dkl_deklar_user varchar(255), dkl_deklar_user_id uuid, dkl_modif_date timestamp(3), dkl_modif_user varchar(255), dkl_modif_user_id uuid, CONSTRAINT g9_deklaravimas_pkey PRIMARY KEY (dkl_id));
...
--Public Views:	
	...
	CREATE OR REPLACE VIEW public.v_rodikliai AS SELECT rod_id as "ID", rod_grupe as "Grupe", rod_kodas as "Kodas", rod_rodiklis as "Pavadinimas", rod_daznumas as "Daznumas", rod_min as "Min", rod_max as "Max", rod_step as "Step", rod_vnt as "Vnt", rod_aprasymas as "Aprasymas" FROM public.rodikliai;
	CREATE OR REPLACE VIEW public.v_reiksmes AS SELECT rks_id as "ID", rks_deklar as "Deklaracija", rks_rodiklis as "Rodiklis", rks_date as "Data", rks_reiksme as "Reiksme" FROM public.reiksmes ORDER BY rks_rodiklis,rks_date;
	CREATE OR REPLACE VIEW public.v_daznumas AS SELECT dzn_grupe as "Daznumas", dzn_nuo as "Nuo", dzn_iki as "Iki", dzn_kartai as "Kartai", dzn_laikas as "Laikas", l.val as "Daugiklis" FROM public.daznumas as d left join public.lkp_daznumo_daugiklis as l on (d.dzn_laikas=l.key) ORDER BY dzn_id ASC;
	CREATE OR REPLACE VIEW public.lkp_rodikliu_grupes AS SELECT lkp_num AS key, lkp_value AS val FROM public.lookup WHERE lkp_group::text = 'RodikliuGrupe'::text;
	CREATE OR REPLACE VIEW public.lkp_rodikliu_validacija AS SELECT lkp_num AS key, lkp_value AS val FROM lookup WHERE lkp_group = 'RodikliuGrupe' and lkp_key='Taip';
	CREATE OR REPLACE VIEW public.lkp_stebesenos AS SELECT lkp_num as key, lkp_value as val FROM public.lookup WHERE lkp_group='Stebesenos';
	CREATE OR REPLACE VIEW public.lkp_statusas AS SELECT lkp_num as key, lkp_value as val FROM public.lookup WHERE lkp_group='Statusas';
	CREATE OR REPLACE VIEW public.lkp_daznumas AS SELECT lkp_key as key, lkp_value as val FROM public.lookup WHERE lkp_group='Daznumas';
	CREATE OR REPLACE VIEW public.lkp_daznumo_laikas AS SELECT lkp_num as key, lkp_value as val FROM public.lookup WHERE lkp_group='DaznumoLaikas';
	CREATE OR REPLACE VIEW public.lkp_daznumo_daugiklis AS SELECT lkp_num as key, try_cast(lkp_key,0::integer) as val FROM public.lookup WHERE lkp_group='DaznumoLaikas';
	GRANT SELECT ON TABLE public.v_gvts TO g9_app; GRANT SELECT ON TABLE public.v_deklar TO g9_app; GRANT SELECT ON TABLE public.v_gvts_ja TO g9_app; GRANT SELECT ON TABLE public.lkp_rodikliu_grupes TO g9_app; GRANT SELECT ON TABLE public.lkp_stebesenos TO g9_app;GRANT SELECT ON TABLE public.lkp_statusas TO g9_app; GRANT SELECT ON TABLE public.v_rodikliai TO g9_app;GRANT SELECT ON TABLE public.lkp_daznumas TO g9_app; GRANT SELECT ON TABLE public.lkp_daznumo_laikas TO g9_app; GRANT SELECT ON TABLE public.lkp_daznumo_daugiklis TO g9_app; GRANT SELECT ON TABLE public.lkp_rodikliu_validacija TO g9_app; GRANT SELECT ON TABLE public.v_daznumas TO g9_app; GRANT SELECT ON TABLE public.v_reiksmes TO g9_app;
...
--Funkcijos:	
	CREATE OR REPLACE FUNCTION public.try_cast(_in text, INOUT _out ANYELEMENT) LANGUAGE plpgsql AS $$ BEGIN EXECUTE format('SELECT %L::%s', $1, pg_typeof(_out)) INTO _out; EXCEPTION WHEN others THEN END $$;
	CREATE OR REPLACE FUNCTION public.gvts_users(gvts bigint[])
	...
	CREATE OR REPLACE FUNCTION public.valid_kartojasi(deklar integer) RETURNS TABLE(rodiklis integer, data date, reiksme double precision) LANGUAGE 'plpgsql' AS $$ BEGIN
	...
	CREATE OR REPLACE FUNCTION public.valid_kartojasi_get(deklar integer) RETURNS TABLE("ID" integer, "Rodiklis" integer, "Data" date, "Reiksme" double precision, "Patvirtinta" boolean, "Pastabos" text) LANGUAGE 'plpgsql' AS $$
	...
	CREATE OR REPLACE FUNCTION public.valid_kartojasi_set(deklar integer, userid uuid) RETURNS TABLE("ID" integer, "Rodiklis" integer, "Data" date, "Reiksme" double precision, "Patvirtinta" boolean, "Pastabos" text) LANGUAGE 'plpgsql' AS $$ BEGIN
	...
	CREATE OR REPLACE FUNCTION public.valid_trukumas_get(deklar integer) RETURNS TABLE("ID" integer, "Rodiklis" integer, "Suvesta" integer, "Reikia" integer, "Patvirtinta" boolean, "Pastabos" text) LANGUAGE 'plpgsql' AS $$BEGIN RETURN QUERY
	...
	CREATE OR REPLACE FUNCTION public.valid_trukumas_set(deklar integer, userid uuid) RETURNS TABLE("ID" integer, "Rodiklis" integer, "Suvesta" integer, "Reikia" integer, "Patvirtinta" boolean, "Pastabos" text) LANGUAGE 'plpgsql' AS $$ BEGIN
	...
	CREATE OR REPLACE FUNCTION public.valid_virsija_set(deklar integer, userid uuid) RETURNS TABLE("ID" integer, "Rodiklis" integer, "Nuo" date, "Iki" date, "Max" double precision, "Detales" boolean, "Nereiksmingas" boolean, "NereiksmApras" text, "Zmones" integer, "Tipas" varchar, "LOQReiksme" varchar, "LOQVerte" varchar, "Statusas" varchar, "Patvirtinta" boolean, "Pastabos" text) LANGUAGE 'plpgsql' AS $$ BEGIN
